    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                @if($data->type=="add")
                <form action="/dashboard/tag" method="post" class="p-4">
                    @else
                    <form action="/dashboard/tag/{{$data->data->id}}" method="post" class="p-4">
                        @method("put")
                        @endif
                        @csrf
                        <div class="grid lg:grid-cols-2 gap-4">
                            <div class="">
                                ...
                                <input value="{{old('nama',$data->data->nama)}}" type="text" name="nama" placeholder="Nama tag" class="px-4 slug py-2 w-full text-white rounded-md bg-gray-700" id="nama">
                                @error("nama")
                            ...
                            <div class="">
                                ...
                                <input value="{{old('slug',$data->data->slug)}}" type="text" name="slug" placeholder="Slug tag" class="px-4 slug-target py-2 w-full text-white rounded-md bg-gray-700" id="slug">
                                @error("slug")
                                <small class="text-red-500">{{$message}}</small>
                                @enderror
                            </div>
