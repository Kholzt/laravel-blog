<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="/dashboard/tag/create" class="px-4 py-2 bg-blue-500 text-white rounded-md ">Tambah Tag</a>
        </div>
    </x-slot>
                ...
                <table class="text-white table-auto w-full ">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 border-b border-slate-500">{{$tag->nama}}</td>
                            <td class="px-4 py-2 border-b border-slate-500">
                                <a href="/dashboard/tag/{{$tag->id}}/edit" class="text-yellow-500 hover:text-yellow-500/70 text-sm"><i class="fa-solid fa-pencil me-1"></i> Ubah</a>
                                <button data-toggle=".modal" data-id="{{$tag->id}}" data-nama="{{$tag->nama}}" class="ms-2 deleteBtn text-red-500 hover:text-red-500/70 text-sm"><i class="fa fa-trash me-1"></i> Hapus</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                            ...
                            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <div class="">
                                    <div class="mt-3 text-center  sm:mt-0 sm:text-left">
                                        <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Hapus Tag</h3>
                                        <div class="mt-2">
                                            <p class="text-sm text-gray-500 ">Apakah anda yakin menghapus tag <span class="tag-name font-bold"></span>? Tag akan dihapus secara permanent dan tidak dapat di kembalikan</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".deleteBtn").on("click", function() {
            ...
            $("form").attr("action", "/dashboard/tag/" + id)
            $(".tag-name").text(nama)
        })
    })
</script>